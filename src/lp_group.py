...
def dynamic_grouping(delay_matrix):
    """
    在 3 到 N 范围内动态确定最优的组数量，优化 makespan。
    """
    n = len(delay_matrix)  # 获取节点数
    best_makespan = float('inf')  # 初始化最优 makespan
    best_group_plan = None  # 保存最优分组方案

    for n_clusters in range(3, n + 1):  # 遍历组数，从 3 到 n
        print(f"尝试分为 {n_clusters} 组...")
        group_plan = optimize_grouping(delay_matrix, n_clusters)

        if group_plan:
            makespan = max(max(delay_matrix[i][j] for i in group for j in group) for group in group_plan)  # 计算组内最大延迟
            print(f"组数 {n_clusters} 的 makespan: {makespan}")

            # 如果当前 makespan 小于已知最优值，则更新
            if makespan < best_makespan:
                best_makespan = makespan
                best_group_plan = group_plan

    print(f"找到的最优 makespan: {best_makespan}")
    return best_group_plan
...
def main():
    ...
    # 调用动态分组算法
    group_plan = dynamic_grouping(delay_matrix)
